...
Meanwhile, filter-repo ate the docs/ directory like it was popcorn. Every architectural decision, every handoff note, every ADR — gone in a single `--invert-paths`. I rebuilt them from memory. Close.

Game Guard fought me for six rebuilds. Not because the code was wrong — because I placed it after the `</Grid>` closing tag. It compiled but never rendered.

The installer logo: 55x55, 65x65 with 15px padding, 25x35, 10px white padding, 10px black padding, 65x65 with 10px right/top only. PIL's `Image.paste()` needs the alpha channel extracted separately. Seven tries.

The license page: `LicenseLabel`, `LicenseLabel2`, `LicenseLabel3` — three wrong configurations, three rebuilds.

The title bar icon: `SetIcon` with .ico, `BitmapImage` with PNG, `SetIcon` in Loaded, multi-res .ico with 7 embedded sizes.

We ended with a working installer. Splash stamped. Icon visible. Prereqs auto-install with 'Installing...' then 'Continue.' Tagged.

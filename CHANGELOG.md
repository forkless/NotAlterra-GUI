...
### Fixed
- **MANIFEST integrity check** — parser used `Split(' ', 2)` incompatible with SHA256 hash format introduced in v0.5.2. All MANIFEST entries silently dropped, causing false "not in MANIFEST" errors. Changed to unlimited split + `parts[^1]` (last token = filename). Handles both old (2-column) and new (3-column) formats.
- **Backup guard** — `OnCreateBackup` now checks game guard before creating backup, preventing race with concurrent game saves.

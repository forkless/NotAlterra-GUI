...
public class GvasParserTests
{
    ...
    private static byte[] BuildIntPropertyBytes(string propName, uint value)
    {
        var nameBytes = System.Text.Encoding.UTF8.GetBytes(propName);
        var typeBytes = System.Text.Encoding.UTF8.GetBytes("IntProperty");
    ...
    private static byte[] BuildDoublePropertyBytes(string propName, double value)
    {
        var nameBytes = System.Text.Encoding.UTF8.GetBytes(propName);
        var typeBytes = System.Text.Encoding.UTF8.GetBytes("DoubleProperty");
    ...
    // ── IntProperty extraction ────────────────────────────────────────

    [Fact]
    public void ExtractFullMetadata_IntProperty_FindsBuildNumber()
    {
        var slotData = BuildMinimalGvasBytes("SlotName", "savegame_0");
        var intData = BuildIntPropertyBytes("BuildNumber", 12345);
        var combined = new byte[] { (byte)'G' }
            .Concat(slotData).Concat(intData)
            .ToArray();

        var path = WriteTempFile(combined);
        try
        {
            var meta = GvasParser.ExtractFullMetadata(path);
            Assert.Null(meta.CorruptionReason);
            Assert.Equal((uint)12345, meta.BuildNumber);
        }
        finally { File.Delete(path); }
    }

    [Fact]
    public void ExtractFullMetadata_IntProperty_FindsSavesCount()
    {
        var slotData = BuildMinimalGvasBytes("SlotName", "savegame_0");
        var intData = BuildIntPropertyBytes("SavesCount", 42);
        var combined = new byte[] { (byte)'G' }
            .Concat(slotData).Concat(intData)
            .ToArray();
        ...
        {
            var meta = GvasParser.ExtractFullMetadata(path);
            Assert.Equal((uint)42, meta.SavesCount);
        }
        finally { File.Delete(path); }
    }

    [Fact]
    public void ExtractFullMetadata_IntProperty_FindsLatestVersion()
    {
        var slotData = BuildMinimalGvasBytes("SlotName", "savegame_0");
        var intData = BuildIntPropertyBytes("LatestVersion", 7);
        var combined = new byte[] { (byte)'G' }
            .Concat(slotData).Concat(intData)
            .ToArray();
        ...
        {
            var meta = GvasParser.ExtractFullMetadata(path);
            Assert.Equal((uint)7, meta.LatestVersion);
        }
        finally { File.Delete(path); }
    }

    [Fact]
    public void ExtractFullMetadata_IntProperty_FindsDataVersion()
    {
        var slotData = BuildMinimalGvasBytes("SlotName", "savegame_0");
        var intData = BuildIntPropertyBytes("DataVersion", 99);
        var combined = new byte[] { (byte)'G' }
            .Concat(slotData).Concat(intData)
            .ToArray();
        ...
        {
            var meta = GvasParser.ExtractFullMetadata(path);
            Assert.Equal((uint)99, meta.DataVersion);
        }
        finally { File.Delete(path); }
    }

    [Fact]
    public void ExtractFullMetadata_IntProperty_Absent_ReturnsNull()
    {
        var slotData = BuildMinimalGvasBytes("SlotName", "savegame_0");
        var combined = new byte[] { (byte)'G' }.Concat(slotData).ToArray();

        var path = WriteTempFile(combined);
        try
        {
            var meta = GvasParser.ExtractFullMetadata(path);
            Assert.Null(meta.BuildNumber);
            Assert.Null(meta.SavesCount);
            Assert.Null(meta.LatestVersion);
            Assert.Null(meta.DataVersion);
        }
        finally { File.Delete(path); }
    }

    // ── DoubleProperty (playtime) ─────────────────────────────────────

    [Fact]
    public void ExtractFullMetadata_DoubleProperty_FindsPlaytime()
    {
        // Build "Elapsed" text followed by a plausible double nearby
        // The scanner looks for "Elapsed" byte marker, then scans offsets 8-50 for a plausible double
        var slotData = BuildMinimalGvasBytes("SlotName", "savegame_0");
        var elapsed = "Elapsed"u8.ToArray();
        var playtimeBytes = BitConverter.GetBytes(12345.67);
        // Put "Elapsed" then some padding then the double value
        var playtimeBlock = elapsed.Concat(new byte[12]).Concat(playtimeBytes).ToArray();
        var combined = new byte[] { (byte)'G' }
            .Concat(slotData).Concat(playtimeBlock)
            .ToArray();
        ...
        {
            var meta = GvasParser.ExtractFullMetadata(path);
            Assert.NotNull(meta.PlaytimeSeconds);
            Assert.Equal(12345.67, meta.PlaytimeSeconds.Value, precision: 2);
        }
        finally { File.Delete(path); }
    }

    [Fact]
    public void ExtractFullMetadata_DoubleProperty_PlaytimeAbsent_ReturnsNull()
    {
        var slotData = BuildMinimalGvasBytes("SlotName", "savegame_0");
        var combined = new byte[] { (byte)'G' }.Concat(slotData).ToArray();
